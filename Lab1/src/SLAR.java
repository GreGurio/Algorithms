...
public class SLAR extends Matrix implements SLARinterface {
    ...
    public SLAR(int[] freeArg, int[][] matrix) throws IllegalArgumentException {
        if (freeArg.length != 3 && matrix.length != 3 && matrix[0].length != 3)
            throw new IllegalArgumentException("СЛАУ или столбец свободных членов не подходит по размеру. Они должны быть формата 3х3 и 3х1 соответственно.");
        this.matrix = matrix;
